...
$back_act='';
if($user_id){
$m_sql = 'SELECT is_validated FROM ' . $ecs->table('users') . ' WHERE user_id='.$user_id;
$user_validated = current(current($db->getAll($m_sql)));
}
...
if ($action == 'default')
{   if($user_validated=='0'){
         show_message($_LANG['check_email']);
 }
...
elseif ($action == 'act_register')
{
    ...
    else
    {
		...
		$tuijianren   = isset($_POST['tuijianren']) ? trim($_POST['tuijianren']) : '';
        ...
        if (register($username, $password, $email, $other, $tuijianren) !== false)
        {
            /*把新注册用户的扩展信息插入数据库*/
