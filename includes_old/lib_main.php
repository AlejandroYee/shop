...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    /* 初始化“页面标题”和“当前位置” */
    $page_title = $GLOBALS['_CFG']['shop_title'] . ' - ' . 'Design By LegouDay';
...
function get_navigator($ctype = '', $catlist = array())
{
    ...
    foreach($navlist['middle'] as $k=>$v)
    {
		...
		$rs = strpos($v['url'],"category");
$navlist['middle'][$k]['cat_arr'] = array();
if($rs!==false)
{
preg_match("/\d+/i",$v['url'],$matches);
$cid = $matches[0];
$navlist['middle'][$k]['cat_arr'] = cat_list($cid, 0, false);
}
